...
import { 
    Calendar, Users, FileText, CheckCircle, BarChart2, Plus, 
    MoreHorizontal, Clock, AlertCircle, Download, Trash2, 
    Filter, Search, Mail, Phone, ExternalLink, Target, TrendingUp, DollarSign
} from 'lucide-react';
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between shrink-0">
        <div>
           <div className="flex items-center gap-3 mb-2">
              <h1 className="text-2xl font-bold text-white">E-Ticaret Platformu Yenileme</h1>
              <span className="px-3 py-1 bg-green-500/20 text-green-400 text-xs rounded-full font-medium border border-green-500/30">Aktif</span>
           </div>
           <p className="text-gray-400 text-sm">Proje Yöneticisi: Emre Yılmaz • Başlangıç: 10 Ocak 2023</p>
        </div>
        <div className="mt-4 md:mt-0 flex items-center gap-4">
             <div className="flex -space-x-2">
                {teamMembers.slice(0, 4).map(member => (
                    <img key={member.id} src={`https://picsum.photos/id/${member.avatar}/40/40`} className="w-8 h-8 rounded-full border-2 border-dark-900" alt="member" />
                ))}
